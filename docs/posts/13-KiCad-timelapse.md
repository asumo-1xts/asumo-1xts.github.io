...
| 引数 | 説明                             | デフォルト         |
| ---- | -------------------------------- | ------------------ |
| #1   | レイヤー選択（`F_Cu` or `B_Cu`） | `F_Cu`             |
| #2   | 入力ファイルへの相対パス         | `sample.kicad_pcb` |
| #3   | 出力する動画ファイルの名前部分   | `sample`           |
...
::: tip
当然ですが、基板の規模によっては（特にSVG→PNGの処理に）膨大なリソースと時間を要します。私の割と強いデスクトップPCでも冒頭のArduino UNOの処理に8分ほどかかっているので、お手元の計算機の性能には十分に留意してください。
:::
